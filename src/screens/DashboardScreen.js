...
import { getExpenses, exportToExcel } from '../utils/storage';
...
export default function DashboardScreen({ navigation }) {
    ...
    React.useEffect(() => {
        ...
        // Most repeated purpose
        const purposeCounts = {};
        filteredData.forEach(item => {
            purposeCounts[item.purpose] = (purposeCounts[item.purpose] || 0) + 1;
        });
        // Check if purposeCounts is empty before reducing
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={[styles.header, Platform.OS === 'android' && { marginTop: 40 }]}>
                <Text style={theme.typography.header}>Dashboard</Text>
...
const styles = StyleSheet.create({
    ...
    summaryCard: {
        ...
        backgroundColor: theme.colors.surface, // Kept surface for consistency
        borderWidth: 1,
        borderColor: theme.colors.primary, // Pop with border color
    },
    totalAmount: {
        fontSize: 48,
        fontWeight: '800',
